...
module.exports = {
  ...
  // Add inputs to the modal
    ...
    const reply = new EmbedBuilder()
      ...
      .setImage('https://cdn.discordapp.com/attachments/1217561386624876736/1219153708622024816/image.png?ex=660a445e&is=65f7cf5e&hm=b895bb43c347740204b1ad6d34fcc418761b2ec25032ee1412102b6e715953df&')
      .setColor('Blurple')
      .setTimestamp();
      ...
      message.reply({
        ...
        try {
          const confirmation = await msg.awaitMessageComponent({ filter: collectorFilter });

          if (confirmation.customId === 'confirm' ||  confirmation.customId === 'reattempt') {
            ...
            try{
              const submission = await confirmation.awaitModalSubmit({ filter, time: 15_000 });
                ...
                if (submission.isModalSubmit()){
                  ...
                  await submission.update({ content: 'Your submission was received successfully!' });
                }
               ...
               transactionDetails = await getTransactionDetails();
                ...
                if(transactionDetails === 0){
                  ...
                  .addFields({name:"Transaction Hash or ID entered could be wrong or the transaction is not completed yet",value : 'After completion of transaction reattempt the payment process'})

                  


                  const row = new ActionRowBuilder()
                  .addComponents(reattempt);

                  
                  
              
                  await msg.edit({embeds:[transactionReply],components:[row]});
                  continue;
                }
                else{

                const istTimeConvertor = (utcTimestamp) => {
                ...
                function convertToEpochTime(isoTimestamp) {
                  ...
                  const epochTime = date.getTime(); // Returns epoch time in milliseconds
                  
                  return epochTime;
              }
              
              // Assuming transactionDetails.recieved contains the ISO timestamp string
              const epochTime = convertToEpochTime(transactionDetails.recieved);
              
              // No need to convert epochTime to Number, as it's already a number
              const transactionReply = new EmbedBuilder()
                  .setTitle('Transaction Details')
                  .addFields({ name: 'Received From Address : ', value: `0x${transactionDetails.addresses[0]}` })
                  .setDescription(`<t:${epochTime}>`)
                  .addFields({ name: 'Received Date', value: `${istTimeConvertor(transactionDetails.recieved)}` });
              
                  await msg.edit({embeds:[transactionReply],components:[]});

...
module.exports = {
  ...
  async execute(bot, message, args) {
    ...
    if (results.length > 1) {
      ...
      message.reply({ embeds: [reply] }).then(async (msg) => {
        ...
        const filter = (reaction, user) => {
          return (
            numEmojis.includes(reaction.emoji.name) &&
        ...
        collector.on("collect", async (reaction, user) => {
          ...
          const detailreply = new EmbedBuilder()
            .setAuthor({
              ...
              iconURL: `${livePriceApi.png32}`,
            })
            .setColor(`${livePriceApi.color}`)
            .setTitle(`Price: ${Number(selectedCoin.price).toFixed(4)}`)
            .addFields({
              name: "\nℹ️ Details",
              value: `\`\`\`All Time High      : ${Number(
                livePriceApi.allTimeHighUSD
              ).toFixed(4)}\nCirculating Supply : ${millify(
                Number(results.circulatingSupply)
              )}\nMax Supply         : ${millify(
                livePriceApi.maxSupply
              )}\nVolume             : ${millify(livePriceApi.volume)}\`\`\``,
              inline: true,
            })

...
module.exports = {
  ...
  aliases: ["fiat", "f"],
  description:
    "Shows the list of currencies available and there respective codes",
  usage: "[Currency] ",
  async execute(bot, message, args) {
    let currency = "";
    if (args.length > 0) currency = args[0].toLowerCase();
    // console.log('in');
    ...
    if (currency === "") {
      searchedCurrency = results.filter((s) =>
        prominentCurrencies.some((c) =>
          s.code.toLowerCase().includes(c.toLowerCase())
        )
      );
    } else {
      searchedCurrency = results.filter(
        (s) =>
          currency.toLowerCase().includes(s.code.toLowerCase()) ||
          s.name.toLowerCase().includes(currency.toLowerCase())
      );
    }
